describe 'A default Racket test Application' do
  ...
  it 'has mapped controllers correctly' do
    ...
    app.router.action_cache.fetch(DefaultRootController).length.should.equal(5)
    app.router.action_cache.fetch(DefaultRootController).include?(:index).should.equal(true)
    app.router.action_cache.fetch(DefaultRootController).include?(:my_first_route)
      .should.equal(true)
    app.router.action_cache.fetch(DefaultRootController).include?(:my_second_route)
      .should.equal(true)
    app.router.action_cache.fetch(DefaultSubController1).length.should.equal(4)
    app.router.action_cache.fetch(DefaultSubController1).include?(:route_to_root).should.equal(true)
    app.router.action_cache.fetch(DefaultSubController1).include?(:route_to_nonexisting)
      .should.equal(true)
    app.router.action_cache.fetch(DefaultSubController2).length.should.equal(5)
    app.router.action_cache.fetch(DefaultSubController2).include?(:index).should.equal(true)
    app.router.action_cache.fetch(DefaultSubController2).include?(:current_action)
      .should.equal(true)
    app.router.action_cache.fetch(DefaultSubController2).include?(:current_params)
      .should.equal(true)
    app.router.action_cache.fetch(DefaultSubController3).should.equal([:index])
    app.router.action_cache.fetch(DefaultInheritedController).should.equal([:index])
  end
